...
function Mahasiswa() {
  ...
  // Fetch mahasiswa data from the endpoint using axios
  useEffect(() => {
    axios
      .get("https://jsonplaceholder.typicode.com/posts")
      .then((response) => {
        // Adjust the data from the API to match your table format
        const mahasiswaData = response.data.map((item) => ({
          id: item.id,
          nim: item.userId.toString(), // Assuming userId as NIM
          nama: item.title, // Using title as nama
        }));
  ...
  const handleSaveEdit = (e) => {
    e.preventDefault();
    setMahasiswa(
      mahasiswa.map((mhs) => (mhs.id === editData.id ? editData : mhs))
    );
    closeEditModal();
    Swal.fire({
      title: "Sukses!",
