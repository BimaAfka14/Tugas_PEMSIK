...
function AdminLayout() {
  const userRole = localStorage.getItem("role"); // Cek role pengguna dari localStorage

  return (
    <div className="bg-gray-100 min-h-screen flex font-sans">
      <Sider userRole={userRole} /> {/* Saring menu berdasarkan role */}
      <div className="flex-1 flex flex-col">
        <Header />
        <main className="flex-grow bg-white shadow-sm p-6">
          <Outlet /> {/* Menampilkan konten berdasarkan role */}
        </main>
        <Footer />
      </div>
    </div>
    ...
    <aside className="w-64 bg-blue-900 text-white flex flex-col">
      ...
      <nav className="flex-grow px-4 py-6">
        <ul className="space-y-4">
          {/* Menampilkan Dashboard untuk semua role */}
          <NavItem to="/admin" icon="📊" label="Dashboard" />

          {userRole === "admin" && (
            <NavItem to="/admin/mahasiswa" icon="🎓" label="Mahasiswa" />
          )}
          <NavItem to="/admin/setting" icon="⚙️" label="Setting" />
        </ul>
      </nav>
    </aside>
  );
  ...
  const handleLogout = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        localStorage.removeItem("role"); // Hapus role setelah logout
        navigate("/"); // Gunakan navigate untuk kembali ke halaman login
      }
    });
  };
